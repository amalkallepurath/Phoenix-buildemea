...
function Details() {
    ...
    useEffect(() => {
        axios.get("http://localhost/emea/hostelfeedetails.php").then(response => {
            setdata((prev) => {
    ...
    const search=(e)=> {
        ...
        axios.post("http://localhost/emea/hostelfee.php",dat).then(response => {
            setdata((prev) => {
                return { ...prev, result: response.data }
            });
      })
    ...
    return (
        <div>
            ...
            <div id="checkmenu">
                <input type='text' name='name' placeholder='Enter name' onChange={take} />
            ...
            <h1 align="center">Hostel fee details</h1><br/>
                ...
                <td>{item.id}</td>
                  <td>{item.name}</td>
                   <td>{item.slno}</td>
                    ...
                    <td>{item.totalhostelfee}</td>
                    <td>{item.hostelfee}</td>
                    <td>{item.nowhostelfee}</td>
                    <td>{item.phone}</td>
                    <td><Link to={`/updatehostelfee/${item.slno}`} id="btn">Edit</Link></td>
              </tr>
            )
          })
        }
